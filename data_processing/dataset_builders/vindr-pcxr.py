...
def process_single_file(args):
    ...
    p = "vindr-pcxr-an-open-large-scale-pediatric-chest-x-ray-dataset-for-interpretation-of-common-thoracic-diseases-1.0.0/"
    try:
        jpg_path = os.path.join(output_dir, filename.replace('.dicom', '.jpg')).replace(p, "")
        temp_jpg_path = os.path.join(output_dir, 'temp', filename.replace('.dicom', '.jpg')).replace(p, "")
